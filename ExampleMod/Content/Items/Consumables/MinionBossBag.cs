...
namespace ExampleMod.Content.Items.Consumables
{
	...
	public class MinionBossBag : ModItem
	{
		...
		public override bool PreDrawInWorld(SpriteBatch spriteBatch, ref Vector2 screenPosition, ref Rectangle frame, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI) {
			...
			for (float i = 0f; i < 1f; i += 0.25f) {
				...
				spriteBatch.Draw(texture, screenPosition + new Vector2(0f, 8f).RotatedBy(radians) * time, frame, new Color(90, 70, 255, 50), rotation, frameOrigin, scale, SpriteEffects.None, 0);
			}

			for (float i = 0f; i < 1f; i += 0.34f) {
				float radians = (i + timer) * MathHelper.TwoPi;

				spriteBatch.Draw(texture, screenPosition + new Vector2(0f, 4f).RotatedBy(radians) * time, frame, new Color(140, 120, 255, 77), rotation, frameOrigin, scale, SpriteEffects.None, 0);
			}
